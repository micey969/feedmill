...
$currentStmt = $conn->prepare('SELECT full_name, username, job_title, image_name, active_flag, admin_flag FROM accounts WHERE user_id = ?');
...
if ($password !== '') {
    ...
    $stmt = $conn->prepare('UPDATE accounts SET full_name = ?, username = ?, job_title = ?, image_name = ?, password = ?, active_flag = ?, admin_flag = ? WHERE user_id = ?');
    $bindTypes = 'sssssiii';
} else {
    $stmt = $conn->prepare('UPDATE accounts SET full_name = ?, username = ?, job_title = ?, image_name = ?, active_flag = ?, admin_flag = ? WHERE user_id = ?');
    $bindTypes = 'ssssiii';
}
...
if ($password !== '') {
    $stmt->bind_param($bindTypes, $fullName, $username, $jobTitle, $imageName, $password, $activeFlag, $adminFlag, $userId);
} else {
    $stmt->bind_param($bindTypes, $fullName, $username, $jobTitle, $imageName, $activeFlag, $adminFlag, $userId);
}
